...
c....for narrow-bands...
      ...
      anb(nx)=arndm(nx) 
      bnb(nx)=brndm(nx) 
      centnb(nx)=haf*(bandlo(nx)+bandhi(nx)) 
      delnb(nx)=bandhi(nx)-bandlo(nx)
      betanb(nx)=betad(nx)
101   continue
...
c---begin temp. loop (on i) 
      ...
      x1(i)=exp(x(i)) 
      srcs(i)=c1/(x1(i)-one)
      srcwd(i,nx)=srcwd(i,nx)+srcs(i)*dnusb(nsb)
215   continue
213   continue
211   continue
...
c   and dsrce 
      do 221 nx=1,40 
      do 221 i=1,28 
      source(i,iband(nx))=source(i,iband(nx))+srcwd(i,nx)
221   continue
      do 223 nx=9,nbly 
      do 223 i=1,28 
      source(i,nx)=srcwd(i,nx+32) 
223   continue
      do 225 nx=1,nbly 
      do 225 i=1,27 
      dsrce(i,nx)=(source(i+1,nx)-source(i,nx))*hp1
225   continue
      do 231 nx=1,nblw 
      alfanb(nx)=bnb(nx)*anb(nx) 
      arotnb(nx)=sqrt(alfanb(nx)) 
231   continue
...
305      continue 
      if (ia.eq.2) then 
         do 307 i=1,28
         src1nb(i,nx)=del*sc(i)
         dbdtnb(i,nx)=del*dsc(i) 
307      continue 
      endif 
303   continue
301   continue
...
c   sum6,sum7,sum8
      if (cent.lt.560. .or. cent.gt.1200..and.cent.le.2200.) then 
         do 315 i=1,28
         sum4(i)=sum4(i)+src1nb(i,nx)
         sum6(i)=sum6(i)+dbdtnb(i,nx)
         sum7(i)=sum7(i)+dbdtnb(i,nx)*arotnb(nx)
         sum8(i)=sum8(i)+dbdtnb(i,nx)*alfanb(nx)
315      continue 
      endif 
c***perform summations over 160-560 cm-1 freq range for e1 calcs (sum4wd
      if (cent.gt.160. .and. cent.lt.560.) then 
         do 316 i=1,28
         sum4wd(i)=sum4wd(i)+src1nb(i,nx)
316      continue 
      endif 
313   continue
      do 317 i=1,28 
...
417      continue 
         do 419 j=1,180 
         do 419 i=1,28
         sum(i,j)=sum(i,j)+src1nb(i,nx)*expo(j)
         pertsm(i,j)=pertsm(i,j)+dbdtnb(i,nx)*expo(j)
419      continue 
         do 421 j=121,180 
         do 421 i=1,28
         sum3(i,j)=sum3(i,j)+dbdtnb(i,nx)*fac(j) 
421      continue 
      endif 
c---compute sum over 160-560 cm-1 range for use in e1 calcs (sumwde)
      if (cent.gt.160. .and. cent.lt.560.) then 
         do 420 j=1,180 
         do 420 i=1,28
         sumwde(i,j)=sumwde(i,j)+src1nb(i,nx)*expo(j)
420      continue 
      endif 
411   continue
      do 431 j=1,180
